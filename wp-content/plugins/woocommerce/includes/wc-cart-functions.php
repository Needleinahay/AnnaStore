...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		...
		$message   = sprintf( '<a href="%s" class="button wc-forward forwardToCart">%s</a> %s', esc_url( $return_to ), esc_html__( 'Continue shopping', 'woocommerce' ), esc_html( $added_text ) );
	} else {
		$message   = sprintf( '<a href="%s" class="button wc-forward forwardToCart">%s</a> %s', esc_url( wc_get_page_permalink( 'cart' ) ), esc_html__( 'View cart', 'woocommerce' ), esc_html( $added_text ) );
	}
